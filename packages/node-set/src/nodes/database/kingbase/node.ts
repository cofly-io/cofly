...
export class KingbaseES implements INode {
	node: INodeBasic = {
		kind: 'kingbase',
		name: 'KingbaseES数据库',
		event: "kingbase",
	...
	detail: INodeDetail = {
		fields: [
			...
			// 自定义SQL
			{
				...
				AIhelp: {
					enable: true,
					rules: '[你一个KingbaseES的DBA，擅长编写SQL语句，要求：\n1. SQL语句是完整可执行的\n2. 请确保SQL语句正确且逻辑清晰]'
				}
			},
			{
				label: '返回条数',
		...
		try {
			...
			if (client) {
				try {
					...
					console.error('⚠️ [KingbaseES Node] 关闭连接时出错:', closeError.message);
				}
			}
		}
	}
	...
	private async createConnection(inputs: any): Promise<any> {
		...
		if (inputs.datasource) {
			try {
				...
				console.error('❌ [KingbaseES Node] 查询连接配置失败:', error.message);
		...
		try {
			const kb = await eval("import('kb')");
			const client = new kb.Client(connectionConfig);
			await client.connect();
			return client;
		} catch (error: any) {
			console.error('📍 [KingbaseES Node] 连接错误堆栈:', error.stack);
			throw error;
		}
	}

	private async executeSelect(client: any, opts: IExecuteOptions): Promise<any> {
	...
	private async executeInsert(client: any, opts: IExecuteOptions): Promise<any> {
		const table = opts.inputs?.table;
			...
			const values = Object.keys(record).map(key => {
				...
				if (typeof value === 'string' && isoRegex.test(value)) {
					...
					if (!isNaN(date.getTime())) {
						// 转换为KingbaseES datetime格式：'YYYY-MM-DD HH:MM:SS'
	...
	private async executeUpdate(client: any, opts: IExecuteOptions): Promise<any> {
		const table = opts.inputs?.table;
	...
	private async executeCustomQuery(client: any, opts: IExecuteOptions): Promise<any> {
