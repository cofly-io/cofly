...
export class WordNode implements INode {
	...
	private async executeRead(opts: IExecuteOptions): Promise<any> {
		...
		const outputFormat = opts.inputs?.outputFormat || 'html';
		const filterImages = opts.inputs?.filterImages || false;
		...
		try {
			// 使用更稳健的方式读取Word文档
			// 首先读取文件为buffer，然后使用mammoth处理buffer
			const fileBuffer = fs.readFileSync(filePath);
			
			// 配置mammoth选项
			const mammothOptions: any = { buffer: fileBuffer };
			
			// 如果需要过滤图片
			if (filterImages) {
				mammothOptions.convertImage = mammoth.images.imgElement(function(element: any) {
					return element; // 返回原始元素，这样图片就会被保留
				});
			}
			...
			if (outputFormat === 'html') {
				...
				// 检查mammoth是否支持convertToMarkdown
				if (typeof (mammoth as any).convertToMarkdown === 'function') {
					result = await (mammoth as any).convertToMarkdown(mammothOptions);
				} else {
					// 如果不支持，先转为HTML再转为Markdown
					const htmlResult = await mammoth.convertToHtml(mammothOptions);
					// 简单的HTML到Markdown转换
					result = {
						value: this.htmlToMarkdown(htmlResult.value),
				...
				const htmlResult = await mammoth.convertToHtml(mammothOptions);
				result = {
					value: this.htmlToJson(htmlResult.value),
					messages: htmlResult.messages
				};
			} else {
				// 默认使用HTML格式
	...
	private htmlToMarkdown(html: string): string {
		...
		markdown = markdown.replace(/<ol>(.*?)<\/ol>/gi, (match, content) => {
			...
			return items.map((item: string, index: number) => {
				return `${index + 1}. ${item.replace(/<\/?li>/gi, '')}`;
			}).join('\n') + '\n';
		});
