...
export interface WxWorkConfig extends WxWorkNS.Agent {
    token: string;
    aesKey: string;
    baseUrl?: string;
}
...
export interface WxWorkMessage {
    ...
    deptIds?: number[];
    tagIds?: number[];
    type: 'text' | 'markdowm' | 'image' | 'voice' | 'video' | 'location' | 'link' | 'event';
    content?: string;
}

export interface WxWorkVerifyParams {
    msgSignature: string;
    timestamp: string;
    nonce: string;
    echostr?: string;
...
class WXBizMsgCrypt {
    ...
    private decrypt(
        ...
        // 获取尾部填充部分的长度
        const padLen = buffer.slice(-1)[0] || 0;
    ...
    verifyURL(
        ...
        return this.decrypt(msgSignature, timestamp, nonce, echostr);
    }
...
export class WxWork {
    ...
    readonly #config: WxWorkConfig;

    constructor(config: WxWorkConfig) {

        this.#bin = new WxWorkBin(undefined, config.baseUrl);
        this.#config = config;
    }

    /**
     * 验证URL签名
     * @param params 微信发送的验签参数
     * @return 解密以后得 echostr
     */
    verifyURL(params: WxWorkVerifyParams): string | undefined {
        const wxCrypt = new WXBizMsgCrypt(this.#config.token, this.#config.aesKey, this.#config.corpId);
        try {
            // 使用wechat-crypto库的verifyURL方法
            const result = wxCrypt.verifyURL(params.msgSignature, params.timestamp, params.nonce, params.echostr || '');
            return result;
        } catch (error) {
    ...
    decryptMsg(params: WxWorkVerifyParams) {
        const wxCrypt = new WXBizMsgCrypt(this.#config.token, this.#config.aesKey, this.#config.corpId);
        try {
            // 使用wechat-crypto库的verifyURL方法
            const result = wxCrypt.decryptMsg(params.msgSignature, params.timestamp, params.nonce, params.postData || '');
            return result;
        } catch (error) {
            console.error('解密消息文本失败:', error);
            return undefined;
        }
    }

    sendMsg(msg: WxWorkMessage) {
        const svc = new WxWorkMessageService(this.#bin, this.#config);
        if(msg.type === 'text') {
            return svc.text({
                userIds: msg.userIds as [],
                deptIds: msg.deptIds,
                tagIds: msg.tagIds
            }, msg.content || '');
        }
