...
export class Network<T extends StateData> {
  ...
  /**
   * history config for managing threads creation and persistence
   * used to create a new threads, load initial results/history and
...
export class NetworkRun<T extends StateData> extends Network<T> {
  ...
  private async execute(
    ...
    while (
      ...
      const call = await agent.run(`RND#${this._counter + 1}:${agent.name}`, input, { network: this, maxIter: 0 });
      this._counter += 1;
  ...
  private async getNextAgentsViaRoutingAgent(
    ...
    const result = await routingAgent.run(routingAgent.name, input, {
      network: this,
