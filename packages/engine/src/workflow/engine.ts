...
export class Engine {
  ...
  /**
   * Returns all actions added to the workflow
   *
   */
  ...
  /**
   * Replaces all actions in the current workflow
   *
   */
  ...
  graph(flow: Workflow): DAG {
    for (let action of flow.actions) {
      ...
      // Validate that the action kind exists within the workflow.
      if (!this.#actionKinds.has(action.kind)) {
        throw new Error("Workflow instance references unknown action kind: " + action.kind);
      }

      // Validate that the workflow action's input McpInterfaces.ts match the expected
      // McpInterfaces.ts defined on the workflow's action
   ...
   *
   */
  run = async ({ event, step, workflow, state, publish }: RunArgs): Promise<ExecutionState> => {
    ...
    if(state) {
      state.forEach((value, key) => {
        executionState.state.set(key, value);
      })
    }

    await executionState.execute();
...
export class ExecutionState {
  ...
  execute = async () => {
    const { event, step, graph, workflow, engine, publish } = this.#opts;

    const executed = new Set();

    if(workflow.state) {
      for(let key in workflow.state) {
        const value = workflow.state[key];
        this.#state.set(key, value);
      }
    }

    await bfs(graph, async (action, edge) : Promise<ActionResult> => {
      ...
      this.#state.set(action.id, {
          data: result.data
          //text: typeof result.data === "object" ? JSON.stringify(result.data) : result.data,
      });

      this.#state.set("$current", result.data);
      this.#state.set("$input", result.data);
