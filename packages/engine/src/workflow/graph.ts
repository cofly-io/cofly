...
export const bfs = async (graph: DAG, cb: (node: WorkflowAction, edge: WorkflowEdge) => Promise<ActionResult>): Promise<any> => {
  if (graph.order <= 1) {
  ...
  while (queue.length > 0) {
    ...
    for (let node of nodes) {
      ...
      const result = await cb(node.action, edge.value.attributes.edge as WorkflowEdge);
