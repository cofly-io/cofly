...
import { convertExpression } from "../utils/jsonpath";
...
export const resolveInputs = (
  ...
  for (let key in (inputs ?? {})) {
    outputs[key] = resolveInput(inputs[key], vars);
  }
  return outputs
}

export function resolveInput(value: any, vars: Record<string, any>): any {
  ...
  try {
    ...
    if(exp.indexOf("{{") < exp.indexOf("}}")) {
        const matches = exp.match(/\{\{[\s\S]*?\}\}/g);
        if(matches && matches.length > 0) {
            let newValue = value;
            for (const match of matches) {
                const path = convertExpression(match.trim().slice(2, -2).trim());
                try {
                    const item = JSONPath.query(props, path)?.pop() || undefined;
                    if(item && typeof item === "object") {
                        newValue = newValue.replaceAll(match, JSON.stringify(item));
                    } else {
                        newValue = newValue.replaceAll(match, item);
                    }
                } catch(error) {
                    console.log(error);
                }
    ...
    throw Error(`can not render from value: ${JSON.stringify(value)}`)
  }
}
